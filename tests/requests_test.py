...
class BookServiceTest(unittest.TestCase):
    ...
    # curl -i http://localhost:9090/books/1
    def test_find_by_id(self):
        # Exercise
        response = requests.get('http://localhost:9090/books/1', timeout=5)
    ...
    def test_find_all(self):
        response = requests.get('http://localhost:9090/books', timeout=5)
        self.assertEqual(200, response.status_code)
        self.assertEqual('application/json', response.headers['content-type'])
        print(response.json())

    # curl -i -X POST http://localhost:9090/books -H "Content-Type: application/json" 
    ...
    def test_insert(self):
        ...
        response = requests.post('http://localhost:9090/books', timeout=5, json=payload)
        self.assertEqual(201, response.status_code)

    # curl -i -X PUT http://localhost:9090/books/2 -H "Content-Type: application/json" 
    ...
    def test_update(self):
        ...
        response = requests.put('http://localhost:9090/books/2', timeout=5, json=payload)
        self.assertEqual(200, response.status_code)
